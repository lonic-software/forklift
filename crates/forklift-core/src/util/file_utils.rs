...
mod tests {
    ...
    //
    // Checks against `crate::util::remote_utils::PRESENCE_ALLOWANCE_MS_PER_OP` directly — the
    // production constant `RemoteClient::presence_negotiation_budget` actually prices — rather
    // than a local mirror literal: a spike whose ceiling is a second, unlinked copy of the
    // production number looks checked and is not, since changing production would leave the
    // spike passing against the old value.
    // -----------------------------------------------------------------------------------
    #[test]
    #[ignore]
    fn spike_fork92_presence_rate() {
        ...
        use crate::util::remote_utils::PRESENCE_ALLOWANCE_MS_PER_OP;
        ...
        for &n in &[10usize, 100, 1_000, 10_000] {
            ...
            assert!(
                loose_present_ms_per_op < PRESENCE_ALLOWANCE_MS_PER_OP,
                "loose-present at n={}: {:.4}ms/op exceeds the production {}ms/op ceiling",
                n, loose_present_ms_per_op, PRESENCE_ALLOWANCE_MS_PER_OP,
            );
            ...
            assert!(
                packed_present_ms_per_op < PRESENCE_ALLOWANCE_MS_PER_OP,
                "packed-present at n={}: {:.4}ms/op exceeds the production {}ms/op ceiling",
                n, packed_present_ms_per_op, PRESENCE_ALLOWANCE_MS_PER_OP,
            );
            ...
            assert!(
                packed_absent_ms_per_op < PRESENCE_ALLOWANCE_MS_PER_OP,
                "packed-absent at n={}: {:.4}ms/op exceeds the production {}ms/op ceiling",
                n, packed_absent_ms_per_op, PRESENCE_ALLOWANCE_MS_PER_OP,
            );
