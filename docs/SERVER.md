...
| 5 | Office privilege | `verify_office_privileges`, `server.rs:2119` | only for an office-pallet update, only once trusted | each office-modifying parcel is well-formed and authorized: signed by a key tracked at that point, with a parent, in a readable chain; its key-permanence obligations honored (a key is never removed or altered, and a revocation is never undone or added without a reason — binds admins too); and its *signer* held the office role (or self-service right) it needed as of that parcel's own signing (`crates/forklift-aws-lambda/src/error.rs:21-29`) | applies **only** to the office pallet's own chain — never to a working pallet's content or transport |
